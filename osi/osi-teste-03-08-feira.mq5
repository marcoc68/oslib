...
// Alteracoes em relacao a versao 02-06
// Informar volume de venda e de compra
// Periodo para calculo das medias passa a ser em segundos
// Parametro que define se gera ou nao instrucoes sql no log
...
input bool   GERAR_SQL_BOOK        = false ; // se true, gera instrucoes sql no log para gravar o book de ofertas em tabelas postgres
...
#define OSI_FEIRA_SHORT_NAME "osi-03-07-feira"
...
#property description "osi-03-07-feira: Informar volume buy e sell, parametro para gerar sql no log, periodo passa a ser em segundos. "
//#property description "---------------"
...
int OnInit() {
   ...
   SetIndexBuffer(27,m_bufVolBuy     , INDICATOR_DATA  );
   SetIndexBuffer(28,m_bufVolSel     , INDICATOR_DATA  );
...
//--- ticks
 ...
 //m_minion.initialize(QTD_SEGUNDOS_CALC_MEDIA*m_qtd_sec_periodo); // quantidade de segundos que serao usados no calculo das medias.
...
void calcSinalTendencia( const double& vpm[], const double& vpmax[], const double& vpmin[] ){
   ...
   for(int i=0; i<QTD_SEGUNDOS_CALC_MEDIA; i++ ){
