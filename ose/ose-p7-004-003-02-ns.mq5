...
//input ENUM_TIPO_OPERACAO         EA_ACAO_POSICAO            = NAO_ABRIR_POSICAO       ; //*EA_ACAO_POSICAO:Forma de operacao do EA.
  input ENUM_TIPO_ENTRADA_PERMITDA EA_TIPO_ENTRADA_PERMITIDA  = ENTRADA_BUY             ; //*TIPO_ENTRADA_PERMITIDA Tipos de entrada permitidas (sel,buy,ambas e nenhuma)
...
//input int    EA_QTD_SEG_MEDIA_PRECO      = 900      ; // QTD_SEG_MEDIA_PRECO
  ...
  input bool   EA_NEGOCIAR_ATIVO_2         = true     ; // NEGOCIAR_ATIVO_2
  ...
  #define EA_PROCESSAR_BOOK   false //PROCESSAR_BOOK true: obtem dados do book.
  //input bool   EA_PROCESSAR_BOOK   = false; //PROCESSAR_BOOK true: obtem dados do book.

  #define EA_BOOK_DEEP1       3    //BOOK_DEEP1 profundidade book imbalance
  ...
  input double EA_PORC_REGIAO_OPERACIONAL_CANAL  = 0.10;//PORC_REGIAO_OPERACIONAL_CANAL regiao de operacao
...
//input int    EA_MIN_DELTA_VOL_ACELERACAO  = 1   ;//MIN_DELTA_VOL_ACELERACAO:Aceleracao minima da %delta vol para entrar na posicao
  ...
  input int    EA_LAG_RAJADA1                             = 1    ;//LAG_RAJADA1
  input int    EA_LAG_RAJADA2                             = 1    ;//LAG_RAJADA2
  ...
  #define      EA_TOLERANCIA_ENTRADA         1      //*TOLERANCIA_ENTRADA: algumas estrategias permitem uma tolerancia do preco em ticks para entrada na posicao
  ...
  input double EA_QTD_TICKS_4_GAIN_INI_1   = 5    ; //*TICKS_4_GAIN_INI_1 Qtd ticks para o gain qd vol/seg eh level 1;
  input double EA_QTD_TICKS_4_GAIN_INI_2   = 2    ; //*TICKS_4_GAIN_INI_2 Qtd ticks para o gain qd vol/seg eh level 1;
...
input ulong  EA_MAGIC             =  21060700400302; //MAGIC: Numero magico desse EA. yy-mm-vv-vvv-vvv-vv.
////
...
input int    EA_QTD_MILISEG_TIMER  =  2000;//*QTD_MILISEG_TIMER:Tempo de acionamento do timer.
...
MqlDateTime                m_date;
string                     m_name = "OSE-P7-004-003-02-ns"; // operacao manual assistida por robo
...
void inicializarBookEvent(){
     ...
     m_book2.initialize(m_symb_str2,EA_BOOK_DEEP2,EA_BOOK_IMBALANCE2);
}
...
void doCloseOposite( double toClosePriceIn, double vol, string symbol, ENUM_DEAL_TYPE typeDeal ){
    Print(__FUNCTION__,"(",toClosePriceIn,",",vol,",",symbol,",", EnumToString(typeDeal),")" );
...
void OnBookEvent(const string &symbol){
   ...
   if( !EA_PROCESSAR_BOOK ) return;
   if(EA_NEGOCIAR_ATIVO_1) m_book1.setBook();
   if(EA_NEGOCIAR_ATIVO_2) m_book2.setBook();

   //if( symbol == m_symb_str1 ){ m_book1.setBook(); return; }
   //if( symbol == m_symb_str2 ){ m_book2.setBook(); return; }
