...
//|                                                                  |
//| Versao p7-004-001                                                |
//|    Superficie para operacao em pares de ativos.                  |
//|                                                                  |
//| Versao p7-004-001-008                                            |
//|   -Correcao de bugs da estrategia de trade em pares.             |
//|                                                                  |
//|                                                                  |
//|    24/03/2021                                                    |
//|   -Metodo para trazer ordens de saida afastadas para proximo ao  |
//|    breakeven.                                                    |
//|                                                                  |
//|   -Metodo para determinar o t4g em funcao do tamanho do canal.   |
//|                                                                  |
//|    25/03/2021                                                    |
//|   -Comprar quando acionar a tecla C                              |
//|   -Vender  quando acionar a tecla V                              |
//|                                                                  |
//|                                                                  |
//|                                                                  |
//| 2. Estrategia de colocacao de ordens contra a posicao passa      |
//|    a ter parametro de %                                          |
//|                                                                  |
//|    2.2 HFT_FORMADOR_DE_MERCADO                                   |
//|        Abre posicao na direcao da tendencia, quando:             |
//|          - sinal C e H sao verdadeiros ao mesmo tempo.           |
//|          - sinal C na direcao contraria eh falso.                |
//|                                                                  |
//|        Stop posicao quando:                                      |
//|          - sinal C na direcao da posicao eh falso.               |
//|          - sinal C e H na direcao contraria eh verdadeiro.       |
//|                                                                  |
//| 3. VERSOES                                                       |
//|    GERAL                                                         |
//|    * Voltamos a calcular o stop_level assim: 22/10/2020          |
//|      stp_lev_ask = bid + stop_level;                             |
//|      stp_lev_bid = ask - stop_level;                             |
//|                                                                  |
//|      - estava calculando assim:                                  |
//|        stp_lev_ask = ask + stop_level;                           |
//|        stp_lev_bid = bid - stop_level;                           |
//|                                                                  |
//|    * Passamos a calcular o lucro da posicao com base nos precos  |
//|      ask e bid. O anterior, do sistema, eh baseado no preco last |
//|      e causava acionamento do stop_gain esperando lucro, mas     |
//|      levando prejuizo por conta da diferenca de precos.          |
//|                                                                  |
//|    * passamos a diminuir quantidade de ticks para o ganho somente|
//|      quando o breakeven se mover em direcao desfavoravel. Antes  |
//|      alterava mesmo que a movida fosse em direcao favoravel. Ex: |
//|      durante a saida de lotes da posicao.                        |
//|                                                                  |
//|    V4                                                            |
//|    - coloca ordem de entrada e saida ao mesmo tempo. Nao usa     |
//|      doCloseRajada (atualmente soh no gerenciamento da posicao)  |
//|    - nao usa stop.                                               |
//|                                                                  |
//|                                                                  |
//|                                                                  |
...
//input ENUM_TIPO_OPERACAO         EA_ACAO_POSICAO            = HFT_OPERAR_CANAL_EM_PAR ; //*EA_ACAO_POSICAO:Forma de operacao do EA.
  ...
  input int                        SLEEP_TESTE                =  1000                   ; //*ERROOOOOOOOOOOOO SLEEP_TESTE nao pode em producao
...
//
  ...
  input string EA_TICKER_REF          = "WDOJ21" ;//*TICKER_REF
//input int    EA_QTD_SEG_MEDIA_PRECO = 900      ;// QTD_SEG_MEDIA_PRECO
  input int    EA_QTD_PERIODO_MEDIA_SPREAD = 15  ;//*QTD_PERIODO_MEDIA_SPREAD
  ...
  input int    EA_EST_QTD_SEGUNDOS            = 120   ; //EST_QTD_SEGUNDOS 0(qtd seg do timeframe) >0(qtd seg)   
  ...
  #define      EA_BOOK_OUT           0.0    //double BOOK_OUT porcentagem do book que nao eh processada

  //input group "canal operacional"
  #define      EA_CANAL_DIARIO                   false //CANAL_DIARIO operacional
  input int    EA_TAMANHO_CANAL                  = 05; //TAMANHO_CANAL operacional em periodos
...
//input int    EA_MIN_DELTA_VOL_ACELERACAO  = 1   ;//MIN_DELTA_VOL_ACELERACAO:Aceleracao minima da %delta vol para entrar na posicao
  ...
  input bool   EA_DECISAO_ENTRADA_COMPRA_VENDA_AUTOMATICA = true;//DECISAO_ENTRADA_COMPRA_VENDA_AUTOMATICA
  #define      EA_FECHA_POSICAO_NO_BREAK_EVEN               false //FECHA_POSICAO_NO_BREAK_EVEN
  input int    EA_AUMENTO_LAG_QTD_FASE1                   = 3     ;//EA_AUMENTO_LAG_QTD_FASE1 
  input double EA_AUMENTO_LAG_POR_LOTE_PENDENTE_FASE_1    = 0.00  ;//*AUMENTO_LAG_POR_LOTE_PENDENTE_1
  input double EA_AUMENTO_LAG_POR_LOTE_PENDENTE_FASE_2    = 0.00  ;//*AUMENTO_LAG_POR_LOTE_PENDENTE_2
  input double EA_AUMENTO_LAG_POR_LOTE_PENDENTE           = 0.00  ;//AUMENTO_LAG_POR_LOTE_PENDENTE_nao_se_usa
  input bool   EA_OFFSET_DINAMICO                         = false ;//OFFSET_DINAMICO 
  input double EA_DIVISOR_OFFSET                          = 0.5  ;//DIVISOR_OFFSET 0.5 (divide offset por 2)
  ...
  input int    EA_VOL_LOTE_INI             = 1    ; //*VOL_LOTE_INI:Vol do lote na abertura de posicao qd vol/seg eh L1.
  input double EA_QTD_TICKS_4_GAIN_INI_1   = 4    ; //*TICKS_4_GAIN_INI_1 Qtd ticks para o gain qd vol/seg eh level 1;
  ...
  input double EA_QTD_TICKS_4_GAIN_MIN_1   = 3    ; //*QTD_TICKS_4_GAIN_MIN_1 menor alvo inicial possivel;
  input double EA_QTD_TICKS_4_GAIN_MIN_2   = 2    ; //*QTD_TICKS_4_GAIN_MIN_2 menor alvo inicial possivel;
  input double EA_PORC_CANAL_T4G_1         = 0.15 ; //*PORC_CANAL_T4G_1 t4g1 como porc do canal operacional
  input double EA_PORC_CANAL_T4G_2         = 0.15 ; //*PORC_CANAL_T4G_2 t4g2 como porc do canal operacional
  ...
  #define      EA_TAMANHO_RAJADA                    1      //TAMANHO_RAJADA;
...
//
  ...
  input bool   EA_STOP_PARCIAL_ATIVAR                  = false; //*STOP_PARCIAL_ATIVAR
  input int    EA_STOP_PARCIAL_FIRE_VOLUME_TOT         = 6    ; //*STOP_PARCIAL_FIRE_VOLUME_TOT
  input double EA_STOP_PARCIAL_FIRE_PORC_LUCRO_POSICAO = 0.8  ; //*STOP_PARCIAL_FIRE_PORC_LUCRO_POSICAO
...
//input int    EA_STOP_TIPO_CONTROLE_RISCO = 1   ; // STOP_TIPO_CONTROLE_RISCO, se 1, controle normal. Se 2, controle por proximidade do breakeven.
  ...
  input bool   EA_STOP_CUSUM             =  false; // STOP_CUSUM: true, cusum dispara stop.
...
input ulong  EA_MAGIC             =  21030700400108; //MAGIC: Numero magico desse EA. yy-mm-vv-vvv-vvv.
////
...
input int    EA_MI_FIM_OPERACAO   = 30; // *Minuto de fim    da operacao;
input int    EA_HR_FECHAR_POSICAO = 17; // *HR_FECHAR_POSICAO fecha todas as posicoes;
input int    EA_MI_FECHAR_POSICAO = 40; // *MI_FECHAR_POSICAO fecha todas as posicoes;
//---------------------------------------------------------------------------------------------
//
// group "sleep e timer"
...
MqlDateTime        m_date;
string             m_name = "OSE-P7-004-001-08-fms-pt"; // formador de mercado simples - pair trading
...
int OnInit(){
  ...
  //m_qtd_exec_oninit++;
    ...
    m_gerentePos1.inicializar(m_symb_str1,EA_MAGIC);
  //m_gerentePos1.setSpread(m_lag_rajada*m_tick_size1);
...
//
        ...
        m_gerentePos2.inicializar(m_symb_str2,EA_MAGIC+1);
      //m_gerentePos2.setSpread  (m_lag_rajada*m_tick_size2);
...
//      if(qtdTicks>0) Print(__FUNCTION__,":-| ",qtdTicks, " historicos processados... Mais novo eh:", ticks[qtdTicks-1].time );
      ...
      //int qtdTicks = CopyTicks(m_symb_str1, ticks    , COPY_TICKS_ALL, 0, EA_EST_QTD_SEGUNDOS);
        ...
        if( qtdTicks1>0 ){
            ...
            Print(__FUNCTION__,":-| ",qtdTicks2, " historicos ",EA_TICKER_REF," processados... Mais novo eh:", ticks2[qtdTicks2-1].time );
...
void refreshMe(){
  ...
  //}
    ...
    m_canal1.refresh(      m_ask,      m_bid);
    m_canal2.refresh(m_tick2.ask,m_tick2.bid);
    ...
    if( m_qtdPosicoes > 0 && PositionSelect(m_symb_str1) ){
...
void showAcao(string acao){
   ...
   Comment(
         ...
         //" \n m_est.getInclinacaoHLTradeSel=" ,m_est.getInclinacaoHLTradeSel(),
           ...
           " \n LAG_RAJADA="               +astIfNeg(EA_LAG_RAJADA              ) + " / " + astIfNeg(m_lag_rajada               ,0) +
...
void definirPasso(){
   ...
   if( EA_ALVO_DINAMICO ){
       ...
       if( EA_ACAO_POSICAO == HFT_FORMADOR_DE_MERCADO ){
           ...
           m_lag_rajada                           = (int)ceil(m_qtd_ticks_4_gain_ini_1);
...
void inicializarBookEvent(){
    ...
    Print(":-| ", __FUNCTION__," ******************************** Inicializando bookevent...");
    m_tem_book = MarketBookAdd( m_symb1.Name() );
    Print(":-| ", __FUNCTION__," m_tem_book   :", m_tem_book       );
...
void inicializarVariaveisRecebidasPorParametro(){
    ...
    m_vol_lote_ini1        = EA_VOL_LOTE_INI      !=0?EA_VOL_LOTE_INI      *m_lots_step1:m_symb1.LotsMin();
    m_vol_lote_ini2        = EA_VOL_LOTE_INI      !=0?EA_VOL_LOTE_INI      *m_lots_step2:m_symb2.LotsMin();
...
#define  EA_MAX_TMP_FECHAM_POSICAO   5 
...
bool podeExecutarOnTick(){
    ...
    if( m_milisec_ontick < m_milisec_ontick_ant + EA_ONTICK_A_CADA_X_MILIS ) return false;
...
void OnTick(){
    ...
    if( m_estah_no_intervalo_de_negociacao && EA_ACAO_POSICAO != NAO_OPERAR ){
...
void executarEstrategia(){
    ...
    switch(EA_ACAO_POSICAO){
      //case HFT_OPERAR_VOLUME_CANAL   : abrirPosicaoHFTVolumeCanal       (); break;
        case HFT_OPERAR_CANAL_EM_PAR   : abrirPosicaoHFTPairsTradingCanal (); break;
        case HFT_FORMADOR_DE_MERCADO   : abrirPosicaoHFTFormadorDeMercado (); break;
        case HFT_ARBITRAGEM_PAR        : abrirPosicaoHFTarbitragemPar     (); break;
      //case HFT_DESBALANC_BOOK        : abrirPosicaoHFTDesbalancBook     (); break;
      //case HFT_FLUXO_ORDENS          : abrirPosicaoHFTfluxoOrdens       (); break;
...
void naoAbrirPosicao(){
    if( estouSemPosicao1() ){
            m_trade1.cancelarOrdensComentadas(m_symb_str1,m_apmb_sel );
            m_trade1.cancelarOrdensComentadas(m_symb_str1,m_apmb_buy );
    }
    if( estouSemPosicao2() ){
            m_trade2.cancelarOrdensComentadas(m_symb_str2,m_apmb_sel );
            m_trade2.cancelarOrdensComentadas(m_symb_str2,m_apmb_buy );
    }
}
...
void doCloseRajada4Simples( double toClosePriceIn, double vol, string symbol ){
    ...
    //    m_lag_rajada = (int)(  ( ( m_est.getPrecoMedBookAsk() - m_est.getPrecoMedBookBid() )/2.0 )/m_tick_size1  );
    //}
...
//  if( estouComprado1() ){
//      sleepTeste(); gerentePos.fireNorteSul( toClosePriceIn,m_lag_rajada+ticksAdd, tick,SLEEP_TESTE, pos_type );
//      return;
//  }
//  
//  if( estouVendido1() ){
...
void abrirPosicaoHFTPairsTradingCanal(){
    ...
    if(estouSemPosicao2()){
        ...
        if( podeEntrarComprando2() ){
            ...
            if(m_precoOrdem2==0){Print(__FUNCTION__,":Preco Ordem de compra ZERADO! VERIFIQUE!"); return;}
        ...
        if( podeEntrarVendendo2() ){
            ...
            if(m_precoOrdem2==0){Print(__FUNCTION__,":Preco Ordem de venda ZERADO! VERIFIQUE!"); return;}
...
bool podeEntrarComprando1(){ return m_tipo_entrada_permitida==ENTRADA_BUY  || m_tipo_entrada_permitida==ENTRADA_TODAS; }
bool podeEntrarVendendo1 (){ return m_tipo_entrada_permitida==ENTRADA_SELL || m_tipo_entrada_permitida==ENTRADA_TODAS; }
bool podeEntrarComprando2(){ return m_tipo_entrada_permitida==ENTRADA_SELL || m_tipo_entrada_permitida==ENTRADA_TODAS; }
bool podeEntrarVendendo2 (){ return m_tipo_entrada_permitida==ENTRADA_BUY  || m_tipo_entrada_permitida==ENTRADA_TODAS; }
...
void OnTradeTransaction( const MqlTradeTransaction& tran,    // transacao
  ...
  //if(tran.symbol != m_symb_str1) return; //20210305: correcao BUG estava abrindo posicao com papel diferente do que estah sendo processado.
    ...
    if( EA_ACAO_POSICAO == NAO_OPERAR     ) return;
    if( EA_ACAO_POSICAO == FECHAR_POSICAO ) return;
  ...
  //if( (toClose || closer) && toClosePriceIn > 0 ){
    if( tran.type == TRADE_TRANSACTION_DEAL_ADD && toClosePriceIn > 0 ){
    ...
    */
    //if( EA_ACAO_POSICAO == HFT_FORMADOR_DE_MERCADO ) abrirPosicaoHFTFormadorDeMercado();
}
...
void OnBookEvent(const string &symbol){
   ...
   if( !EA_PROCESSAR_BOOK ) return;
   if(symbol!=m_symb_str1) return; // garantindo que nao estamos processando o book de outro simbolo,
   
   MqlBookInfo book[];
   if( !MarketBookGet(symbol, book) ) { Print(":-( Falha MarketBookGet. Motivo: ", GetLastError()); return; }
   
   if( m_tamanhoBook==0) m_tamanhoBook = ArraySize(book);
   if( m_tamanhoBook==0) { Print(":-( Falha book vazio. Motivo: ", GetLastError()); return; }

   m_est.addBook( TimeCurrent(), book, m_tamanhoBook,EA_BOOK_OUT, m_symb1.TickSize() );
