...
public:
  ...
  static bool IsMT4Order( const ulong &Ticket )
  {
    ...
    // Что делать в ситуации, когда лимитный ордер был частично исполнен, а затем удален?
...
// #define UNKNOWN_REASON (-1)
  ...
  static bool CheckPositionCommissionComment( void )
  {
    ...
    {
      MT4ORDERS::Order.Commission = ::PositionGetDouble(POSITION_COMMISSION); // Всегда ноль
...
#define TOSTR2(A) #A + " = " + EnumToString(A) + " (" + (string)(A) + ")\n"
  ...
  static void GetHistoryPositionData( const ulong Ticket )
  {
    ...
    // История (OpenTicket и OrderTicket) подгружена, благодаря GetPositionDealIn, - HistorySelectByPosition
      ...
      if (MT4ORDERS::HistorySelectDeal(OpenTicket)) // Обязательно сработает, т.к. OpenTicket гарантированно в истории.
    #endif // MT4ORDERS_FASTHISTORY_OFF
      {
...
*/
    ...
    if (Res)
    {
      ...
      if (::PositionSelectByTicket(::OrderGetInteger(ORDER_TICKET))) // Ордер и его позиция могут быть одновременно - данное условие поможет только на Hedge-счетах
      {
  ...
  static bool SelectByExistingTicket( const long &Ticket )
  {
    ...
    else if (::HistoryDealSelect(Ticket))
    {
      if (MT4HISTORY::IsMT4Deal(Ticket)) // Если сделан выбор по DealOut.
        MT4ORDERS::GetHistoryPositionData(Ticket);
      else if (::PositionSelectByTicket(::HistoryDealGetInteger(Ticket, DEAL_POSITION_ID))) // Выбор по DealIn
        MT4ORDERS::GetPositionData();
      else
        Res = false;
    }
    else if (::HistoryOrderSelect(Ticket) && ::PositionSelectByTicket(::HistoryOrderGetInteger(Ticket, ORDER_POSITION_ID))) // Выбор по тикету MT5-ордера
      MT4ORDERS::GetPositionData();
    else
      Res = false;
