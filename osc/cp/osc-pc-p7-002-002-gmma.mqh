...
//+------------------------------------------------------------------+
//| Class osc_control_panel_p7_002_002                               |
//| Usage: Painel de Controle para o Expert P7-002-002               |
//+------------------------------------------------------------------+
class osc_control_panel_p7_002_002 : public CAppDialog {
private:
   ...
   CLabel            m_labelGMMAInst;                 // the label object
   CEdit             m_editGMMAInst;                  // the display field object
   int               m_GMMAInst;                      // soma das medias das operacoes institucionais

   CLabel            m_labelGMMATrader;               // the label object
   CEdit             m_editGMMATrader;                // the display field object
   int               m_GMMATrader;                    // somas das medias das operacoes dos traders
...
public:
                     osc_control_panel_p7_002_002(void);
                    ~osc_control_panel_p7_002_002(void);
   //--- create
 ...
 //void setVolGMMAInst  (double p ){ if(p==m_GMMAInst      )return; m_GMMAInst     = p; m_editGMMAInst     .Text( DoubleToString (p,0) ); if( p<0)m_editGMMAInst     .Color(clrRed); if(p>0)m_editGMMAInst     .Color(clrBlue); if(p==0)m_editGMMAInst     .Color(clrGray);  }

   //void setGMMAInst  (int p){ if(p==m_GMMAInst  )return; m_GMMAInst  =p; m_editGMMAInst  .Text( IntegerToString (p) ); if( p>0){m_editGMMAInst  .Color(clrBlue); return;}else{ if(p<0){m_editGMMAInst  .Color(clrRed);return;} m_editGMMAInst  .Color(clrGray); } }
   //void setGMMATrader(int p){ if(p==m_GMMATrader)return; m_GMMATrader=p; m_editGMMATrader.Text( IntegerToString (p) ); if( p>0){m_editGMMATrader.Color(clrBlue); return;}else{ if(p<0){m_editGMMATrader.Color(clrRed);return;} m_editGMMATrader.Color(clrGray); } }

   void setGMMAInst  (int p){ if(p==m_GMMAInst  )return; m_GMMAInst  =p; setEditValue(p,m_editGMMAInst  ,0,clrBlue,clrRed,clrGray); }
   void setGMMATrader(int p){ if(p==m_GMMATrader)return; m_GMMATrader=p; setEditValue(p,m_editGMMATrader,0,clrBlue,clrRed,clrGray); }

 //void setPUP3(double p, double lim ){ if(p==m_GMMAInst)return; m_GMMAInst=p; m_editGMMAInst.Text( DoubleToString (p,0) ); if( p>lim)m_editGMMAInst.Color(clrBlue); if(p<lim)m_editGMMAInst.Color(clrRed); if(p==lim)m_editGMMAInst.Color(clrRed); }
 //void setPUP2(double p, double lim ){ if(p==m_GMMATrader)return; m_GMMATrader=p; m_editGMMATrader.Text( DoubleToString (p,0) ); if( p>=lim)m_editGMMATrader.Color(clrBlue); if(p<lim)m_editGMMATrader.Color(clrBlack); }
 //void setPUP1(double p, double lim ){ if(p==m_PUP1)return; m_GMMATrader=p; m_editPUP1.Text( DoubleToString (p,0) ); if( p>=lim)m_editPUP1.Color(clrBlue); if(p<lim)m_editPUP1.Color(clrBlack); }
 //void setPUP0(double p, double lim ){ if(p==m_PUP0)return; m_GMMATrader=p; m_editPUP0.Text( DoubleToString (p,0) ); if( p>=lim)m_editPUP0.Color(clrBlue); if(p<lim)m_editPUP0.Color(clrBlack); }
...
//| Event Handling                                                   |
//+------------------------------------------------------------------+
EVENT_MAP_BEGIN(osc_control_panel_p7_002_002)
...
//| Constructor                                                      |
//+------------------------------------------------------------------+
osc_control_panel_p7_002_002::osc_control_panel_p7_002_002(void):mMail(false),
                                                                 mPush(false),
                                                                 mAlert_(true),
                                                                 mLots(0.1),
                                                                 mTakeProfit(50),
...
//| Destructor                                                       |
//+------------------------------------------------------------------+
osc_control_panel_p7_002_002::~osc_control_panel_p7_002_002(void){}
//+------------------------------------------------------------------+
//| Create                                                           |
//+------------------------------------------------------------------+
bool osc_control_panel_p7_002_002::Create(){ return Create(0,"Painel de Controle P7-002-002",0,100,100,LARGURA_CONTROL_PANEL,ALTURA_CONTROL_PANEL); }
bool osc_control_panel_p7_002_002::Create(const long chart,const string name,const int subwin,const int x1,const int y1,const int x2,const int y2)
  {
...
//--- create dependent controls
 //if(!CreateCheckGroup()) return(false);
   ...
   if(!CreateLabel2(INI_COLUNA_03, LARGURA_COLUNA_03, 0, "GMMAI"  ,m_labelGMMAInst,"GMMAI"                      )) return(false);
   if(!CreateEdit2 (INI_COLUNA_04, LARGURA_COLUNA_04, 0, "GMMAI"  ,m_editGMMAInst ,DoubleToString(m_GMMAInst,0) )) return(false);
   ...
   if(!CreateLabel2(INI_COLUNA_03, LARGURA_COLUNA_03, 1, "GMMAT",m_labelGMMATrader,"GMMAT"                        )) return(false);
   if(!CreateEdit2 (INI_COLUNA_04, LARGURA_COLUNA_04, 1, "GMMAT",m_editGMMATrader ,IntegerToString(m_GMMATrader)  )) return(false);
...
//+------------------------------------------------------------------+
bool osc_control_panel_p7_002_002::CreateCheckGroup(void)
  {
//--- coordinates
   int x1=INDENT_LEFT;
   int y1=INDENT_TOP;
...
//+------------------------------------------------------------------+
bool osc_control_panel_p7_002_002::CreateLabel2(int coluna, int lenColuna, int linha, string nome, CLabel &objLabel, string strValor ){
//--- coordinates
...
//+------------------------------------------------------------------+
bool osc_control_panel_p7_002_002::CreateEdit2(int coluna, int lenColuna, int linha, string nome, CEdit &objEdit, string strValor ){
//--- coordinates
   int x1=INDENT_LEFT+coluna;
   int y1=INDENT_TOP +CONTROLS_GAP_Y+linha*(BUTTON_HEIGHT+CONTROLS_GAP_Y);
   int x2=x1+lenColuna;
...
//+------------------------------------------------------------------+
bool osc_control_panel_p7_002_002::CreateButtonOK(int posicao)
  {
//--- coordinates
   int x1=INDENT_LEFT;
...
//| Event handler                                                    |
//+------------------------------------------------------------------+
void osc_control_panel_p7_002_002::OnChangeCheckGroup(void)
  {
   Comment("Value="+IntegerToString(m_check_group.Value())+
           "\nElement 0 has a state: "+IntegerToString(m_check_group.Check(0))+
           "\nElement 1 has a state: ",IntegerToString(m_check_group.Check(1))+
...
//| Event handler                                                    |
//+------------------------------------------------------------------+
//void osc_control_panel_p7_002_002::OnChangeEditPasso(void)
//  {
...
//| Event handler                                                    |
//+------------------------------------------------------------------+
void osc_control_panel_p7_002_002::OnChangeProfitPosicao(void)
  {
...
//| Event handler                                                    |
//+------------------------------------------------------------------+
void osc_control_panel_p7_002_002::OnChangeEdit3(void)
  {
...
//| Event handler                                                    |
//+------------------------------------------------------------------+
void osc_control_panel_p7_002_002::OnChangeEdit4(void)
  {
...
//| Event handler                                                    |
//+------------------------------------------------------------------+
void osc_control_panel_p7_002_002::OnChangeEdit5(void)
  {
...
//| Event handler                                                    |
//+------------------------------------------------------------------+
void osc_control_panel_p7_002_002::OnChangeEdit6(void)
  {
...
//| Event handler                                                    |
//+------------------------------------------------------------------+
void osc_control_panel_p7_002_002::OnClickButtonOK(void)
  {
//--- verifying changes
...
//| Set check for element                                            |
//+------------------------------------------------------------------+
bool osc_control_panel_p7_002_002::SetCheck(const int idx,const bool check)
  {
...
//| Initialization                                                   |
//+------------------------------------------------------------------+
bool osc_control_panel_p7_002_002::Initialization(const bool Mail,const bool Push,const bool Alert_,
                                     const double Lots,const int TakeProfit,
                                     const int  TrailingStop,const int MACDOpenLevel,
                                     const int  MACDCloseLevel,const int MATrendPeriod)
  {
...
//| Get values                                                       |
//+------------------------------------------------------------------+
void osc_control_panel_p7_002_002::GetValues(bool &Mail,bool &Push,bool &Alert_,
                                double &Lots,int &TakeProfit,
                                int &TrailingStop,int &MACDOpenLevel,
                                int &MACDCloseLevel,int &MATrendPeriod)
  {
...
//|  Send notifications                                              |
//+------------------------------------------------------------------+
void osc_control_panel_p7_002_002::Notifications(const string text)
  {
...
//| Get check for element                                            |
//+------------------------------------------------------------------+
int osc_control_panel_p7_002_002::GetCheck(const int idx)
  {
