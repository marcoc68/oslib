...
//+------------------------------------------------------------------+
//| Class osc_control_panel_p7_002_003                               |
//| Usage: Painel de Controle para o Expert P7-002-003-BB            |
//+------------------------------------------------------------------+
class osc_control_panel_p7_002_003 : public CAppDialog {
private:
   ...
   CLabel            m_labelBBSup;                 // the label object
   CEdit             m_editBBSup;                  // the display field object
   int               m_BBSup;                      // banda superior da bolinguer

   CLabel            m_labelBBMed;               // the label object
   CEdit             m_editBBMed;                // the display field object
   int               m_BBMed;                    // media da bolinguer

   CLabel            m_labelBBInf;                     // the label object
   CEdit             m_editBBInf;                      // the display field object
   double            m_BBInf;                          // banda inferior da bolinguer

   CLabel            m_labelBBDP;                        // the label object
   CEdit             m_editBBDP;                         // the display field object
   double            m_BBDP;                             // desvio padrao da BB em pontos
   ...
   void setEditValue(int p, CEdit &edit, int lim, color cup, color cdw, color clim ){ 
        
        //edit=p; 
        edit.Text( IntegerToString (p) ); 
...
public:
                     osc_control_panel_p7_002_003(void);
                    ~osc_control_panel_p7_002_003(void);
   //--- create
 ...
 //void setVolBBSup  (double p ){ if(p==m_BBSup      )return; m_BBSup     = p; m_editBBSup     .Text( DoubleToString (p,0) ); if( p<0)m_editBBSup     .Color(clrRed); if(p>0)m_editBBSup     .Color(clrBlue); if(p==0)m_editBBSup     .Color(clrGray);  }

   //void setBBSup  (int p){ if(p==m_BBSup  )return; m_BBSup  =p; m_editBBSup  .Text( IntegerToString (p) ); if( p>0){m_editBBSup  .Color(clrBlue); return;}else{ if(p<0){m_editBBSup  .Color(clrRed);return;} m_editBBSup  .Color(clrGray); } }
   //void setBBMed(int p){ if(p==m_BBMed)return; m_BBMed=p; m_editBBMed.Text( IntegerToString (p) ); if( p>0){m_editBBMed.Color(clrBlue); return;}else{ if(p<0){m_editBBMed.Color(clrRed);return;} m_editBBMed.Color(clrGray); } }

   void setBBSup(int p, int limiar=0 ){ if(p==m_BBSup)return; m_BBSup=p; setEditValue(p,m_editBBSup,limiar,clrBlue,clrRed,clrGray); }
   void setBBMed(int p, int limiar=0 ){ if(p==m_BBMed)return; m_BBMed=p; setEditValue(p,m_editBBMed,limiar,clrBlue,clrRed,clrGray); }
   void setBBInf(int p, int limiar=0 ){ if(p==m_BBInf)return; m_BBInf=p; setEditValue(p,m_editBBInf,limiar,clrBlue,clrRed,clrGray); }
   void setBBDP (int p, int limiar=0 ){ if(p==m_BBDP )return; m_BBDP =p; setEditValue(p,m_editBBDP ,limiar,clrBlue,clrRed,clrGray); }
 //void setPDW0 (int p, int limiar=0 ){ if(p==m_PDW0 )return; m_PDW0 =p; setEditValue(p,m_editPDW0 ,limiar,clrBlue,clrRed,clrGray); }
...
//| Event Handling                                                   |
//+------------------------------------------------------------------+
EVENT_MAP_BEGIN(osc_control_panel_p7_002_003)
...
//| Constructor                                                      |
//+------------------------------------------------------------------+
osc_control_panel_p7_002_003::osc_control_panel_p7_002_003(void):mMail(false),
                                                                 mPush(false),
                                                                 mAlert_(true),
                                                                 mLots(0.1),
                                                                 mTakeProfit(50),
...
//| Destructor                                                       |
//+------------------------------------------------------------------+
osc_control_panel_p7_002_003::~osc_control_panel_p7_002_003(void){}
//+------------------------------------------------------------------+
//| Create                                                           |
//+------------------------------------------------------------------+
bool osc_control_panel_p7_002_003::Create(){ return Create(0,"Painel de Controle P7-002-003",0,100,100,LARGURA_CONTROL_PANEL,ALTURA_CONTROL_PANEL); }
bool osc_control_panel_p7_002_003::Create(const long chart,const string name,const int subwin,const int x1,const int y1,const int x2,const int y2)
  {
...
//--- create dependent controls
 //if(!CreateCheckGroup()) return(false);
   ...
   if(!CreateLabel2(INI_COLUNA_03, LARGURA_COLUNA_03, 0, "BBSup"  ,m_labelBBSup  ,"BBSup"                   )) return(false);
   if(!CreateEdit2 (INI_COLUNA_04, LARGURA_COLUNA_04, 0, "BBSup"  ,m_editBBSup   ,DoubleToString(m_BBSup,0) )) return(false);
   ...
   if(!CreateLabel2(INI_COLUNA_03, LARGURA_COLUNA_03, 1, "BBMed",m_labelBBMed,"BBMed"                   )) return(false);
   if(!CreateEdit2 (INI_COLUNA_04, LARGURA_COLUNA_04, 1, "BBMed",m_editBBMed ,IntegerToString(m_BBMed)  )) return(false);
   ...
   if(!CreateLabel2(INI_COLUNA_03, LARGURA_COLUNA_03, 2, "BBInf"       ,m_labelBBInf ,"BBInf"                   )) return(false);
   if(!CreateEdit2 (INI_COLUNA_04, LARGURA_COLUNA_04, 2, "BBInf"       ,m_editBBInf  ,DoubleToString(m_BBInf,0) )) return(false);
   ...
   if(!CreateLabel2(INI_COLUNA_03, LARGURA_COLUNA_03, 3, "BBDP"       ,m_labelBBDP         ,"BBDP"                            )) return(false); // desvio padrao da banda de bolinguer
   if(!CreateEdit2 (INI_COLUNA_04, LARGURA_COLUNA_04, 3, "BBDP"       ,m_editBBDP          ,DoubleToString(m_BBDP,0)          )) return(false); // desvio padrao da banda de bolinguer
...
//+------------------------------------------------------------------+
bool osc_control_panel_p7_002_003::CreateCheckGroup(void)
  {
//--- coordinates
   int x1=INDENT_LEFT;
   int y1=INDENT_TOP;
...
//+------------------------------------------------------------------+
bool osc_control_panel_p7_002_003::CreateLabel2(int coluna, int lenColuna, int linha, string nome, CLabel &objLabel, string strValor ){
//--- coordinates
...
//+------------------------------------------------------------------+
bool osc_control_panel_p7_002_003::CreateEdit2(int coluna, int lenColuna, int linha, string nome, CEdit &objEdit, string strValor ){
//--- coordinates
   int x1=INDENT_LEFT+coluna;
   int y1=INDENT_TOP +CONTROLS_GAP_Y+linha*(BUTTON_HEIGHT+CONTROLS_GAP_Y);
   int x2=x1+lenColuna;
...
//+------------------------------------------------------------------+
bool osc_control_panel_p7_002_003::CreateButtonOK(int posicao)
  {
//--- coordinates
   int x1=INDENT_LEFT;
...
//| Event handler                                                    |
//+------------------------------------------------------------------+
void osc_control_panel_p7_002_003::OnChangeCheckGroup(void)
  {
   Comment("Value="+IntegerToString(m_check_group.Value())+
           "\nElement 0 has a state: "+IntegerToString(m_check_group.Check(0))+
           "\nElement 1 has a state: ",IntegerToString(m_check_group.Check(1))+
...
//| Event handler                                                    |
//+------------------------------------------------------------------+
//void osc_control_panel_p7_002_003::OnChangeEditPasso(void)
//  {
...
//| Event handler                                                    |
//+------------------------------------------------------------------+
void osc_control_panel_p7_002_003::OnChangeProfitPosicao(void)
  {
...
//| Event handler                                                    |
//+------------------------------------------------------------------+
void osc_control_panel_p7_002_003::OnChangeEdit3(void)
  {
...
//| Event handler                                                    |
//+------------------------------------------------------------------+
void osc_control_panel_p7_002_003::OnChangeEdit4(void)
  {
...
//| Event handler                                                    |
//+------------------------------------------------------------------+
void osc_control_panel_p7_002_003::OnChangeEdit5(void)
  {
...
//| Event handler                                                    |
//+------------------------------------------------------------------+
void osc_control_panel_p7_002_003::OnChangeEdit6(void)
  {
...
//| Event handler                                                    |
//+------------------------------------------------------------------+
void osc_control_panel_p7_002_003::OnClickButtonOK(void)
  {
//--- verifying changes
...
//| Set check for element                                            |
//+------------------------------------------------------------------+
bool osc_control_panel_p7_002_003::SetCheck(const int idx,const bool check)
  {
...
//| Initialization                                                   |
//+------------------------------------------------------------------+
bool osc_control_panel_p7_002_003::Initialization(const bool Mail,const bool Push,const bool Alert_,
                                     const double Lots,const int TakeProfit,
                                     const int  TrailingStop,const int MACDOpenLevel,
                                     const int  MACDCloseLevel,const int MATrendPeriod)
  {
...
//| Get values                                                       |
//+------------------------------------------------------------------+
void osc_control_panel_p7_002_003::GetValues(bool &Mail,bool &Push,bool &Alert_,
                                double &Lots,int &TakeProfit,
                                int &TrailingStop,int &MACDOpenLevel,
                                int &MACDCloseLevel,int &MATrendPeriod)
  {
...
//|  Send notifications                                              |
//+------------------------------------------------------------------+
void osc_control_panel_p7_002_003::Notifications(const string text)
  {
...
//| Get check for element                                            |
//+------------------------------------------------------------------+
int osc_control_panel_p7_002_003::GetCheck(const int idx)
  {
